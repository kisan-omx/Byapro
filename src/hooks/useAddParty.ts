...
import { createNewParty, checkPartyExistsByName } from '../services/partyService';
...
export function useAddParty() {
  ...
  const [activeTab, setActiveTab] = useState<'credit_info' | 'additional_details'>('credit_info');
  const [openingBalance, setOpeningBalance] = useState('');
  const [balanceType, setBalanceType] = useState<'To Receive' | 'To Give'>('To Receive');
  ...
  const saveParty = useCallback(
    async (onSuccess?: (createdParty: Party) => void): Promise<Party | null> => {
      ...
      try {
        ...
        const parsedBalance = parseFloat(openingBalance.replace(/[^0-9.]/g, '')) || 0;
        const calcBalanceType: 'To Receive' | 'To Give' | 'Settled' = parsedBalance > 0 ? balanceType : 'Settled';
    ...
    [name, phone, email, address, partyType, openingBalance, balanceType, asOfDate, validate]
  );
  ...
  const selectContact = useCallback((contactName: string, contactPhone?: string) => {
    setName(contactName);
    if (contactPhone) {
