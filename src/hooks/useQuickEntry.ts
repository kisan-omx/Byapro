    ...
    async (
        ...
        case "Purchase": {
          ...
          optimisticInfo = {
            ...
            message: `Rs. ${amountNum.toLocaleString()} has been successfully added as ${
              isCash
                ? "Cash Purchase"
                : `Credit Purchase (${selectedParty!.name})`
            }`,
          };
          break;
        }
        ...
        case "Sale Return": {
          ...
          optimisticInfo = {
            ...
            message: `Rs. ${amountNum.toLocaleString()} recorded as ${
              isCash
                ? "Cash Sale Return"
                : `Sale Return (${selectedParty!.name})`
            }`,
          };
          break;
        }
          ...
          optimisticInfo = {
            ...
            message: `Rs. ${amountNum.toLocaleString()} recorded as ${
              isCash
                ...
                : `Purchase Return (${selectedParty!.name})`
            }`,
          };
          break;
        }
      ...
      (async () => {
        try {
          ...
          if (!businessId) {
            throw new Error(
              "Could not find your business. Please complete setup.",
            );
          }
          ...
          if (createdId) {
            realItem = await fetchSingleTransaction(
              businessId,
              createdId,
              optimisticType,
            );
          ...
          Alert.alert(
            "Save Failed",
            err?.message ||
              "Something went wrong while saving. Tap card to retry.",
          );
        } finally {
          setSaving(false);
