...
const Numpad = React.memo(function Numpad({ onPress, onBackspace, onClear, onCalculate }: NumpadProps) {
  const Button = ({ 
    label, 
    onPressItem, 
    flex = 1, 
    ...
    return (
      <TouchableOpacity
        ...
        style={{ flex, aspectRatio: flex > 1 ? undefined : 1.6, height: flex > 1 ? 'auto' : undefined }}
      >
        {icon ? (
          icon
        ) : (
