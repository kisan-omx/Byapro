  ...
  const renderStatusBadge = () => {
    ...
    return (
      <View className={`px-2.5 py-0.5 rounded-md ${bgClass}`}>
        <Text className={`text-xs font-bold ${textClass}`}>{item.status}</Text>
      </View>
    );
  };
  ...
  return (
    ...
    >
      ...
      <View className="flex-row items-center justify-between mb-0.5">
        <Text
          className={`text-sm font-semibold ${badgeConfig.textClass}`}
          numberOfLines={1}
        >
          {typeLabel}
        </Text>
      ...
      <View className="flex-row items-center justify-between mb-0.5 min-h-[22px]">
        <Text
          className="text-base font-bold text-slate-900 flex-1 pr-2"
          numberOfLines={1}
        >
