...
const SummaryCard = ({ title, subtitle, type }: SummaryCardProps) => {
  ...
  return (
    ...
    >
      <View>
        <Text className={`text-base font-bold mb-1 ${titleColor}`}>{title}</Text>
        <Text className={`text-xs font-medium ${subtitleColor}`}>{subtitle}</Text>
      </View>
...
export default function SummaryCards() {
  return (
    <View className="px-4 flex-row flex-wrap justify-between">
      ...
      <SummaryCard 
        title="Rs. 7,287" 
        subtitle="Sales (Bhadau)" 
        type="default" 
      />
      <SummaryCard 
        title="Rs. 0" 
        subtitle="Purchase (Bhadau)" 
        type="default" 
      />
      <SummaryCard 
        title="Rs. 69" 
        subtitle="Expense (Bhadau)" 
        type="default" 
      />
      <SummaryCard 
        title="Total Balance" 
