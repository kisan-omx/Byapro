...
const PAYMENT_OPTIONS: PaymentTypeOption[] = [
  {
    ...
    renderIcon: () => <MaterialCommunityIcons name="cash-multiple" size={22} color="#16A34A" />,
  },
  {
    id: 'Cheque',
    label: 'Cheque',
    bgColor: 'bg-amber-50',
    renderIcon: () => <FontAwesome5 name="money-check" size={17} color="#D97706" />,
  },
  {
    id: 'Bank',
    label: 'Bank',
    bgColor: 'bg-sky-50',
    renderIcon: () => <MaterialCommunityIcons name="bank" size={20} color="#0284C7" />,
  },
  {
    id: 'Online',
    label: 'Online',
    bgColor: 'bg-purple-50',
    renderIcon: () => <Ionicons name="card-outline" size={20} color="#9333EA" />,
  },
];
          ...
          <TouchableWithoutFeedback>
            <View className="bg-surface rounded-t-3xl w-full pb-8 overflow-hidden shadow-2xl">
              ...
              <View className="flex-row items-center justify-between px-6 py-4 border-b border-slate-100">
                <Text className="text-lg font-bold text-slate-800">Payment Type</Text>
                <TouchableOpacity
                  onPress={onClose}
              ...
              <View className="py-1">
                {PAYMENT_OPTIONS.map((item) => {
                  const isSelected = selectedMethod.toLowerCase() === item.id.toLowerCase();
                  return (
                    <TouchableOpacity
                      key={item.id}
                      onPress={() => {
                    ...
                    >
                      <View className="flex-row items-center">
                        <View className={`w-9 h-9 rounded-xl items-center justify-center mr-3.5 ${item.bgColor}`}>
                          {item.renderIcon()}
                        </View>
