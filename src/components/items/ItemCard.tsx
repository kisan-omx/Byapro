...
export const ItemCard: React.FC<ItemCardProps> = ({
  ...
  // imagePath (from DB) takes priority; imageUrl is the local URI during optimistic display
  // We append ?t=updatedAt to the DB URL to bust the CDN cache when an image is replaced
  ...
  return (
    ...
    >
      ...
      <View className="flex-row items-center mb-2">
        {/* Avatar: image if available, otherwise first letter */}
        <View className="w-12 h-12 rounded-md bg-slate-100 items-center justify-center mr-3 flex-shrink-0 overflow-hidden">
          {displayImageUri ? (
            <Image
      ...
      <View className="flex-row items-center pl-16">
        ...
        <View className="flex-1">
          <Text className="text-[10px] text-text-secondary font-medium mb-0.5">
            Sales
          </Text>
