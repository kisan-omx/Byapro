 ...
 */
export function convertToBS(date: Date): { bsYear: number; bsMonth: string; bsMonthIndex: number; bsDay: number } {
  ...
  if (matchedIdx === 0 && (month < 3 || (month === 3 && day < 14))) {
    if (month === 0 && day < 15) {
      ...
      bsDay = Math.floor((date.getTime() - msDate.getTime()) / (1000 * 60 * 60 * 24)) + 1;
 ...
 */
export function getDateFilterSubtitle(filter: DateFilterType, now = new Date()): string {
  const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
 ...
 */
export function getDateFilterBoundaries(dateFilter: DateFilterType): DateRangeBoundaries {
  const now = new Date();
  ...
  if (dateFilter === 'yesterday') {
    ...
    return { startDateIso: startOfYesterday.toISOString(), endDateIso: startOfToday.toISOString() };
  }
  ...
  if (dateFilter === 'last_month') {
    ...
    return { startDateIso: startOfLastMonth.toISOString(), endDateIso: startOfThisMonth.toISOString() };
  }
  if (dateFilter === 'this_fiscal_year') {
    // Nepali Fiscal Year starts around July 16 / 17
    const currentYear = now.getFullYear();
    const fyStartMonth = 6; // July (0-indexed)
    const fyStartDate = now.getMonth() >= fyStartMonth
