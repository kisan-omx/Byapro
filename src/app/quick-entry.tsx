...
import QuickEntryTabs, {
  EntryType,
} from "../components/quick-entry/QuickEntryTabs";
...
export default function QuickEntryScreen() {
  ...
  const [entryType, setEntryTypeState] = useState<EntryType>(
    () => type || activeDraft.entryType,
  );
  const [amount, setAmountState] = useState<string>(() => activeDraft.amount);
  const [selectedParty, setSelectedPartyState] = useState<Party | null>(
    () => activeDraft.selectedParty,
  );
  ...
  return (
    <View className="flex-1 bg-surface">
      ...
      <QuickEntryTabs selectedTab={entryType} onSelectTab={handleTabChange} />
      ...
      <PartySelectionModal
        ...
        onNavigateToAddParty={() => router.navigate("/add-party")}
      />
